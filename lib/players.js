...
/*
 * Get two random teams comprised of all *active* players.
 */
    ...
    function(playerList)
    {
      ...
      else
      {
        ...
        for (var i = 0; i < playerList.length; i++)
          if (playerList[i].isActive) activePlayers.push(playerList[i]);
        utils.shuffleArray(activePlayers);
        var halfWayPoint = Math.floor(activePlayers.length / 2);
        teams.push(activePlayers.slice(0, halfWayPoint));
        teams.push(activePlayers.slice(halfWayPoint));
        cb(teams);
      }
    }
  );
